...
export class LoginPanel {
  ...
  constructor(host: HTMLElement, onSubmit: (credentials: Credentials) => void) {
    ...
    this.element.innerHTML = `<div class="identity-topline"><span>IXD INTERNAL NETWORK</span><span class="identity-ready">● SYSTEM READY</span></div>
      ...
      <label for="ixd-password">PASSWORD <span>密码</span></label><input id="ixd-password" name="password" type="password" autocomplete="current-password" maxlength="128" placeholder="输入访问密码" aria-describedby="identity-feedback" required />
