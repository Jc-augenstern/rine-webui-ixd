...
| `source`                    | 文档来源与章节，按纯文本显示，不作为链接                     |

以上字段均必填，文本不能只包含空白。所有内容按纯文本显示，不支持 Markdown 或 HTML；引号按 JSON 规则写为 `\"`，换行写为 `\n`。页面会转义 HTML 特殊字符，TXT 下载保留原文。

当前内容来自用户提供的 `IXD社团(2).docx`（《IXD社团》架构方案初稿），整理为社团概览、专业方向、项目实践、竞赛沙龙、招新培养五组，每组八份。下载文件使用 `IXD-X-001.txt` 至 `IXD-X-040.txt`。保留初稿性质、建议规模及典型项目的语义，不将方案写成已完成的成果。原文概述中“五个方向组”与实际六节不一致，卡片按 3.1—3.6 的六个方向整理。未添加联系人、联系方式、具体活动日期或实际成果。
